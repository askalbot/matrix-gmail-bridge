...
import json
import abc
from .crypto import aes_encrypt_str, aes_decrypt_str
import nio
from dataclasses import dataclass
...
class Db:
	...
	async def get_user(self, user_id: str) -> User:
		if (e:= await self.kv.get("u:" + user_id)) is not None:
			return User.parse_raw(aes_decrypt_str(self.encryption_key, e)).narrow()
	...
	async def upsert_user(self, user: User):
		...
		await self.kv.set("u:"+user.matrix_id, aes_encrypt_str(self.encryption_key, user.json()))
